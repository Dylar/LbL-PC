...
public class MainGui extends JFrame implements ControlListener,ActionListener
{
	...
	private void initMainFrame()
	{
		...
		JLabel serverConnection = new JLabel("Server online?");

		JButton connectButton = new JButton("Start Server");// das ist "Start"
		JButton disconnectButton = new JButton("Stop Server");// das ist "Stop"
	...
	private JPanel deviceInformation()
	{
		...
		{
			serverHistory.append("Es sind zu viele Ger�te angemeldet!");
			return null;
		}
	}
	...
	{// TODO actionperformed
		...
		if (e.getActionCommand().equals("ACTION_COMMAND_SEND"))
		{
			...
			ca.setMessage(userInputField.getText());
			tryAction(ca);
		...
		{
			ControlAction ca = getNewAction();
			ca.setAction(ControlHandler.START_SERVER);
			tryAction(ca);
		}
...
//				tryAction(ca);
	...
	public void appendServerHistory(String s)
	{
		serverHistory.append(s);
		serverHistory.append("\n");
	...
	public void setServerHealth(boolean b)
	{
		if (b)
			...
			serverHealth.setBackground(new Color(180, 0, 0));
	}


	public void appendChatMessage(String m)
	{
		final String fromUser = m;
		if (!(fromUser.equals("")))
		{
			Date date = new Date();
	...
	private void menuBar()
	{
		...
		JMenuItem oeffnenEntry = new JMenuItem("�ffnen");
